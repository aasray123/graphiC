...
static void string() {
    emitConstant(C_TO_OBJ_VALUE(copyString(parser.previous.start + 1, 
...
static void number(bool canAssign){
    ...
    emitConstant(C_TO_NUMBER_VALUE(value));
}
...
ParseRule rules[] = {
  //[TOKEN] = {prefix function call, infix/postfix function call, precedence}
  // Punctuation and Operators
  [TOKEN_LEFT_PAREN]    = {grouping, call,    PREC_CALL},
  [TOKEN_RIGHT_PAREN]   = {NULL,     NULL,    PREC_NONE},
  [TOKEN_LEFT_BRACE]    = {NULL,     NULL,    PREC_NONE},
  [TOKEN_RIGHT_BRACE]   = {NULL,     NULL,    PREC_NONE},
  [TOKEN_LEFT_BRACKET]  = {array, indexOp, PREC_CALL},
...
static void expression(){
    //Number literals, parentheses, unary negation, /*-+
...
//Continue after errors in a statement to catch as many as possible
...
static void declareVariable(){
    ...
    for(int i = current->localCount - 1 ; i >= 0; i--) {
        ...
        if(identifiersEqual(name, &local->name)){
            error("Already vairbale with this name in this scope");
        }
    }
    addLocal(*name);
}
...
static void declaration() {
    if(match(TOKEN_ENTITY)) {
        ...
        if (current->type == TYPE_SCRIPT) {
            error("Variable declarations are not allowed at the top-level. Use setup() to define globals.");
            // Parse the rest of the statement to avoid cascade errors, but don't compile it.
            while (!check(TOKEN_SEMICOLON) && !check(TOKEN_EOF)) advance();
            consume(TOKEN_SEMICOLON, "Expect ';' after invalid var declaration.");
        } else {
            varDeclaration();
        }
    }
...
/*
    Declaration calls a function, struct, var or statement
    statement has expression, print, if, while, for, block
    The expression has a value, variable, grouping, or unary/binary operation
